...
- [] Copy the EA Folder Structure from the EA Folder Structure Template
- [] Create Transports
- [] Create Endpoints
- [] Create Adapters
- [] Setup Config
...
```
adapter/                      # Named after the adapter
├─ config/
│  ├─ index.ts               # Custom settings (optional but common)
│  ├─ overrides.json         # Overrides file (optional)
│  └─ includes.json          # Includes file, e.g. inverses (optional)
├─ endpoint/
│  ├─ endpoint_1.ts          # Endpoints definition (named after API endpoint)
│  └─ endpoint_2.ts
├─ transport/
│  └─ transport.ts           # Transports definition (named after endpoint)
├─ index.ts                  # Adapter registration
├─ test/
│  ├─ integration/          # must have
│  ├─ unit/                 # must have
├─ package.json
├─ CHANGELOG.md              # Auto-updated through changesets
├─ README.md                 # Auto-generated by CI scripts
├─ test-payload.json
├─ tsconfig.json
└─ tsconfig.test.json
```

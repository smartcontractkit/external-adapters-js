...
local cpuUsagePanel = graphPanel.new(
  ...
  prometheus.target(
    'sum(rate(process_cpu_seconds_total{' + masterFilter + '}' + interval + ')) * 100',
    legendFormat='cpu'
  )
);
...
local redisConnectionsOpen = graphPanel.new(
  ...
  prometheus.target(
    'redis_connections_open{' + masterFilter + '}',
...
local redisRetriesCount = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'sum(redis_retries_count{' + masterFilter + '})',
...
local redisCommandsSentCount = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'rate(redis_commands_sent_count{' + masterFilter + '}' + interval + ') * 60 ',
...
local heapUsedPanel = graphPanel.new(
  ...
  prometheus.target(
    'nodejs_heap_size_used_bytes{' + masterFilter + '} / 1000 / 1000',
    legendFormat='MB'
  )
);
...
local httpRequestDurationAverageSeconds = graphPanel.new(
  ...
  prometheus.target(
    'rate(http_request_duration_seconds_sum{' + masterFilter + '}' + interval + ')/rate(http_request_duration_seconds_count{' + masterFilter + '}' + interval + ')',
...
local httpRequestDurationSecondsHeatmap = heatmapPanel.new(
  ...
  prometheus.target(
    'sum(increase(http_request_duration_seconds_bucket{' + masterFilter + '}[$__interval])) by (le)',
    legendFormat='{{le}}',
    format='heatmap',
  )
);
...
local wsConnectionActiveGraph = graphPanel.new(
  ...
  prometheus.target(
    'ws_connection_active{' + masterFilter + '}',
...
local wsConnectionErrorsGraph = graphPanel.new(
  ...
  prometheus.target(
    'ws_connection_errors{' + masterFilter + '}',
    legendFormat='{{service}} | Key:{{key}}',
  ),
);
...
local wsConnectionRetriesGraph = graphPanel.new(
  ...
  prometheus.target(
    'ws_connection_retries{' + masterFilter + '}',
...
local wsActiveSubscriptions = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'ws_subscription_active{' + masterFilter + '}',
...
local wsMessagesPerSecondGraph = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'rate(ws_message_total{' + masterFilter + '}' + interval + ')',
...
local cacheFeedValues = statPanel.new(
  ...
  prometheus.target(
    'cache_data_get_values{' + masterFilter + '}',
    legendFormat='{{feed_id}}',
  )
);
...
local cacheMaxAgeSeconds = barGaugePanel.new(
  ...
  prometheus.target(
    'sort_desc(cache_data_max_age{' + masterFilter + '}/1000) != 0',
    instant=true,
    legendFormat='{{feed_id}}',
  )
) + barGaugeConfig;
...
local cacheStalenessSeconds = barGaugePanel.new(
  ...
  prometheus.target(
    'sort_desc(cache_data_staleness_seconds{' + masterFilter + '}) != 0',
    instant=true,
    legendFormat='{{feed_id}}',
  )
) + barGaugeConfig;
...
local cacheEntrySetsPerSecond = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'rate(cache_data_set_count{' + masterFilter + '}' + interval + ')',
    legendFormat='{{feed_id}}',
  )
));
...
local cacheEntryGetsPerSecond = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'rate(cache_data_get_count{' + masterFilter + '}' + interval + ')',
    legendFormat='{{feed_id}}',
  )
));

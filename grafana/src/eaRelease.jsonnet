...
local ErrorRate5xxPanel = graphPanel.new(
  ...
  prometheus.target(
    'topk(10, sum by(app_name) (rate(http_requests_total{' + namespaceFilter + ',status_code=~"^5.."}' + interval + ')) / sum by(app_name) (rate(http_requests_total{app_name!="",' + namespaceFilter + '}' + interval + ')) * 100)'
  )
);
...
local ErrorRate4xxPanel = graphPanel.new(
  ...
  prometheus.target(
    'topk(10, sum by(app_name) (rate(http_requests_total{' + namespaceFilter + ',status_code=~"^4.."}' + interval + ')) / sum by(app_name) (rate(http_requests_total{app_name!="",' + namespaceFilter + '}' + interval + ')) * 100)'
  )
);
...
local Bot10CacheHitTotalReqCount = graphPanel.new(
  ...
  prometheus.target(
    'bottomk(10, (sum(rate(http_requests_total{' + namespaceFilter + ' ,type=\"cacheHit\"}' + interval + ')) by (app_name) / sum(rate(http_requests_total{' + namespaceFilter + ' }' + interval + ')) by (app_name))) * 100'
  )
);
...
local Top10RequestDuration = graphPanel.new(
  ...
  prometheus.target(
    'topk(10, sum(rate(http_request_duration_seconds_sum{' + namespaceFilter + ' }' + interval + ')/rate(http_request_duration_seconds_count{' + namespaceFilter + ' }' + interval + ')) by (app_name))'
  )
);
...
local Provider5xxErrorRate = graphPanel.new(
  ...
  prometheus.target(
    'sum(rate(http_requests_total{' + namespaceFilter + ' ,provider_status_code=~"^5.."}' + interval + ')) by (app_name, provider_status_code)'
  )
);
local Provider4xxErrorRate = graphPanel.new(
  ...
  prometheus.target(
    'sum(rate(http_requests_total{' + namespaceFilter + ' ,provider_status_code=~"^4.."}' + interval + ')) by (app_name, provider_status_code)'
  )
);
...
local Top10CpuUsage = graphPanel.new(
  ...
  prometheus.target(
    'topk(10, sum(rate(process_cpu_seconds_total{' + namespaceFilter + ' }[5m])) by (app_name) * 100)'
  )
);
...
local Top10NodeHeapUsage = graphPanel.new(
  ...
  prometheus.target(
    'topk(10, sum (nodejs_heap_size_used_bytes{' + namespaceFilter + ' ,app_name!=\"REFERENCE_TRANSFORM\"} / 1000 / 1000) by (app_name))'
  )
);
...
local AvgCpuUsage = graphPanel.new(
  ...
  prometheus.target(
    'avg(rate(process_cpu_seconds_total{' + namespaceFilter + ' }[5m])) * 100'
  )
);
...
local AvgNodeHeapUsage = graphPanel.new(
  ...
  prometheus.target(
    'avg(nodejs_heap_size_used_bytes{' + namespaceFilter + ',app_name!=\"REFERENCE_TRANSFORM\"} / 1000 / 1000)'
  )
);
...
local Top10WebsocketSubscriptions = graphPanel.new(
  ...
  prometheus.target(
    'topk(10, sum(ws_connection_active{' + namespaceFilter + '}) by (app_name))'
  )
);
...
local Top10WebsocketSubscriptionsPerSecond = graphPanel.new(
  ...
  prometheus.target(
    'topk(10, sum(rate(ws_subscription_active{' + namespaceFilter + '}' + interval + ')) by (app_name))'
  )
);
...
local Top10WebsocketErrors = graphPanel.new(
  ...
  prometheus.target(
    'topk(10, sum(rate(ws_subscription_errors{' + namespaceFilter + '}' + interval + ')) by (app_name))'
  )
);
...
local Top10CacheWarmerSubscriptions = graphPanel.new(
  ...
  prometheus.target(
    'topk(10, sum(cache_warmer_get_count{' + namespaceFilter + '}) by (app_name))'
  )
);
...
local CacheWarmerErrorRate = graphPanel.new(
  ...
  prometheus.target(
    'sum(rate(http_requests_total{' + namespaceFilter + ',is_cache_warming=\"true\",provider_status_code!=\"undefined\",provider_status_code!=\"200\"}' + interval + ')) by (app_name) / sum(rate(http_requests_total{' + namespaceFilter + ',is_cache_warming=\"true\"}' + interval + ')) by (app_name) * 100'
  )
);

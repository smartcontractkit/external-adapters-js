...
local cpuUsagePanel = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'sum(rate(process_cpu_seconds_total{' + eaSelector + '}' + interval + ') * 100) by (service)',
...
local redisConnectionsOpen = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'sum(redis_connections_open{' + eaSelector + '}) by (service)',
    legendFormat='{{service}}'
  )
));
...
local redisRetriesCount = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'sum(redis_retries_count{' + eaSelector + '}) by (service)',
    legendFormat='{{service}}'
  )
));
...
local redisCommandsSentCount = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'sum(rate(redis_commands_sent_count{' + eaSelector + '}' + interval + ') * 60) by (service)',
    legendFormat='{{service}}'
  )
));
...
local heapUsedPanel = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'sum(nodejs_heap_size_used_bytes{' + eaSelector + '} / 1000 / 1000) by (service)',
    legendFormat='{{service}}'
  )
));
...
local httpRequestsPerMinutePerFeedPanel = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'sum(rate(http_requests_total{feed_id=~"$feed.*",' + eaSelector + '}' + interval + ') * 60) by (feed_id, service)',
...
local httpRequestDurationAverageSeconds = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'sum(rate(http_request_duration_seconds_sum{' + eaSelector + '}' + interval + ') / rate(http_request_duration_seconds_count{' + eaSelector + '}' + interval + ')) by (service)',
    legendFormat='{{service}}',
  )
));
...
local wsConnectionActiveGraph = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'sum(ws_connection_active{' + eaSelector + '}) by (service)',
    legendFormat='{{service}}',
  ),
));
...
local wsConnectionErrorsGraph = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'sum(ws_connection_errors{' + eaSelector + '}) by (service)',
    legendFormat='{{service}}',
  ),
));
...
local wsConnectionRetriesGraph = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'sum(ws_connection_retries{' + eaSelector + '}) by (service)',
    legendFormat='{{service}}',
  )
));
...
local wsActiveSubscriptions = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'sum(ws_subscription_active{' + eaSelector + '}) by (service)',
    legendFormat='{{service}}'
  )
));
...
local wsMessagesPerSecondGraph = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'sum(rate(ws_message_total{' + eaSelector + '}' + interval + ')) by (service)',
    legendFormat='{{service}}'
  )
));
...
local cacheEntrySetsPerSecond = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'sum(rate(cache_data_set_count{' + eaSelector + '}' + interval + ')) by (service)',
    legendFormat='{{service}}',
  )
));
...
local cacheEntryGetsPerSecond = addSideLegend(graphPanel.new(
  ...
  prometheus.target(
    'sum(rate(cache_data_get_count{' + eaSelector + '}' + interval + ')) by (service)',
    legendFormat='{{service}}',
  )
));

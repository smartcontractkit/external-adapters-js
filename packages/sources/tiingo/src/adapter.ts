...
export const makeWSHandler = (config?: Config): MakeWSHandler | undefined => {
  ...
  const isFx = (input: AdapterRequest): boolean =>
    !!input.data.endpoint && endpoints.iex.supportedEndpoints.includes(input.data.endpoint)
  ...
  const isCrypto = (input: AdapterRequest): boolean =>
    !!input.data.endpoint && endpoints.prices.supportedEndpoints.includes(input.data.endpoint)

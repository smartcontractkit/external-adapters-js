...
import {
  ...
  WsTransportTypes,
} from '../../src/transport/stock_quotes-ws'
...
describe('StockQuotesWebSocketTransport', () => {
  ...
  beforeEach(async () => {
    ...
    mockWsServer?.close()
    mockWsServer = new MockWebsocketServer(`${adapterSettings.WS_API_ENDPOINT}/equity/intraday`, {
      mock: false,
    })
  ...
  it('should write response to cache', async () => {
    ...
    socket.send(
      JSON.stringify({
        ...
        service: 'cons',
        data: [
          new Date(providerIndicatedTimeUnixMs).toISOString(),
          symbol,
          0.000933, // liquidity spread
          bid_volume,
          bid_price,
          123.33, // reference price
          ask_price,
          ask_volume,
        ],
      }),
  ...
  it('should use bid as mid when ask is absent', async () => {
    const symbol = 'aapl'
    ...
    const bid_price = 123
    const ask_price = null
    const mid_price = bid_price
    const bid_volume = 5
    const ask_volume = null
    const providerIndicatedTimeUnixMs = 123456789
  ...
  it('should use ask as mid when bid is absent', async () => {
    const symbol = 'aapl'
    ...
    const bid_price = null
    const ask_price = 124
    const mid_price = ask_price
    const bid_volume = null
    const ask_volume = 6
    const providerIndicatedTimeUnixMs = 123456789
  ...
  it('should not write response to cache when bid and ask are absent', async () => {
    const symbol = 'aapl'

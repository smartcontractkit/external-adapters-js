...
describe('websocket', () => {
  ...
  describe('state endpoint', () => {
    ...
    it('should return aggregated_state_price and include timestamp', async () => {
      ...
      // Validate response structure for snapshot
      expect(body.statusCode).toBe(200)
      expect(body.result).toBeDefined()
      expect(body.data).toBeDefined()
      expect(body.timestamps).toBeDefined()
      expect(body.timestamps.providerDataReceivedUnixMs).toBeDefined()
      expect(body.timestamps.providerDataStreamEstablishedUnixMs).toBeDefined()
      expect(body.timestamps.providerIndicatedTimeUnixMs).toBeDefined()

      expect(response.json()).toMatchSnapshot()
    })

    it('should succeed with different quotes', async () => {
      for (const q of ['USD', 'ETH', 'BTC']) {

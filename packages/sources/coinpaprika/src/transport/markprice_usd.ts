...
import { toWsEventType } from './markprice'

const logger = makeLogger('markprice_usd')
...
const normalizeWsSymbol = (symbol: string, exchange: string): string => {
  let normalizedSymbol = symbol
  if (['binance', 'bybit'].includes(exchange)) {
    normalizedSymbol = symbol.replace(/(USDC|USDT)$/, 'USD')
...
export const wsTransport = new WebSocketTransport<WsTransportTypes>({
  url: (context, desiredSubs) => {
    ...
    return `${context.adapterSettings.WS_MARK_PRICE_API_ENDPOINT}?quote=USD`
  },
  ...
  handlers: {
    message(message: WsMessage) {
      ...
      const normalizedWsSymbol = normalizeWsSymbol(message.data.symbol, message.data.exchange)

      if (
        !subscriptions.some(
          (s) =>
            s.exchange === message.data.exchange &&
            s.symbol === normalizedWsSymbol &&

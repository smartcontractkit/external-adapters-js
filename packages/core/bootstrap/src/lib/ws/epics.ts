...
export const connectEpic: Epic<AnyAction, AnyAction, any, any> = (action$, state$) =>
  action$.pipe(
    ...
    filter(([{ connectionKey }, state]) => {
      const isActiveConnection = state.ws.connections[connectionKey].active
      const isConnecting = state.ws.connections[connectionKey].connecting > 1
    ...
    tap(([action, state]) => {
      ...
      switch (action.type) {
        ...
        case disconnected.type:
          if (state.ws.connections[connectionLabels(action.payload).key].wasEverConnected) {

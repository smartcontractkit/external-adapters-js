...
import type { CacheItem, LRUInterface } from './types'
...
export class LocalLRUCache implements ICache {
  ...
  setBatchResponse(batchEntries: { key: string; entry: CacheEntry; maxAge: number }[]): void {
  ...
  ttl(key: string): number {
    ...
    const cacheMap: Map<string, CacheItem> = (this.client as any)[cacheSymbol]

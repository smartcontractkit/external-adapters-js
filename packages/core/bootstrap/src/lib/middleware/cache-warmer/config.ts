...
export function get(): Config {
  const warmupInterval = Number(util.getEnv('WARMUP_INTERVAL'))
  ...
  return {
    ...
    unhealthyThreshold: Number(util.getEnv('WARMUP_UNHEALTHY_THRESHOLD')),
    warmupInterval: Number(util.getEnv('WARMUP_INTERVAL')),
    subscriptionTTL: Number(util.getEnv('WARMUP_SUBSCRIPTION_TTL')),
  }
}

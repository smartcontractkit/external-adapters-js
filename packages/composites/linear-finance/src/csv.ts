  ...
  return new Promise<types.TokenAllocations>((resolve, reject) => {
    parser(input, (err, data: string[][]) => {
      ...
      if (data.length < 5) {
        ...
        Logger.warn(`CSV data has ${data.length} rows, but we expect 9. This data may be malformed!`)
      }

      const numSymbols = data[0].length - 1
      const allocations: types.TokenAllocations = new Array(numSymbols).fill(0)
        .map((_, index) => ({

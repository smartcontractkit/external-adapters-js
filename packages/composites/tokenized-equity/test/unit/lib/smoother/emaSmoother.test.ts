...
describe('EmaSmoother', () => {
  ...
  it('should return raw price when outside transition window', () => {
    ...
    expect(processUpdate('ema', 'test1', 10n, 10n, spread, 100).price).toBe(10n)

    // Should return raw price when before transition window (t=-20)
    expect(processUpdate('ema', 'test1', 20n, 20n, spread, -20).price).toBe(20n)

    // Should return raw price at transition boundary after (t=60)
    expect(processUpdate('ema', 'test1', 30n, 30n, spread, 60).price).toBe(30n)

    // Should return raw price at transition boundary before (t=-10)
    expect(processUpdate('ema', 'test1', 40n, 40n, spread, -10).price).toBe(40n)
  })

  it('should apply smoothing during transition window', () => {
    // Initialize filter with some updates outside window to build EMA state
    processUpdate('ema', 'test2', 100n, 100n, spread, 100)
    processUpdate('ema', 'test2', 100n, 100n, spread, 100)

    // Should be smoothed at t=0 (weight=1.0, fully smoothed)
    const resultAt0 = processUpdate('ema', 'test2', 100n, 100n, spread, 0)
    ...
    const resultAt30 = processUpdate('ema', 'test2', 150n, 150n, spread, 30)
  ...
  it('should handle EMA filter initialization correctly', () => {
    ...
    const firstResult = processUpdate('ema', 'test3', 100n, 100n, spread, 0)
    ...
    jest.setSystemTime(Date.now() + 1000)
    const secondResult = processUpdate('ema', 'test3', 150n, 150n, spread, 0)
  ...
  it('should test various effective alpha (a) values with different time intervals', () => {
    ...
    processUpdate('ema', asset1, initialPrice, initialPrice, spread, 0)
    jest.setSystemTime(baseTime + 50)
    const resultSmallInterval = processUpdate('ema', asset1, newPrice, newPrice, spread, 0)
    ...
    jest.setSystemTime(baseTime)
    processUpdate('ema', asset2, initialPrice, initialPrice, spread, 0)
    jest.setSystemTime(baseTime + 500)
    const resultMediumInterval = processUpdate('ema', asset2, newPrice, newPrice, spread, 0)
    ...
    jest.setSystemTime(baseTime)
    processUpdate('ema', asset3, initialPrice, initialPrice, spread, 0)
    jest.setSystemTime(baseTime + 1000)
    const resultBaseInterval = processUpdate('ema', asset3, newPrice, newPrice, spread, 0)
    ...
    jest.setSystemTime(baseTime)
    processUpdate('ema', asset4, initialPrice, initialPrice, spread, 0)
    jest.setSystemTime(baseTime + 5000)
    const resultLargeInterval = processUpdate('ema', asset4, newPrice, newPrice, spread, 0)
    ...
    jest.setSystemTime(baseTime)
    processUpdate('ema', asset5, initialPrice, initialPrice, spread, 0)
    jest.setSystemTime(baseTime + 10000)
    const resultVeryLargeInterval = processUpdate('ema', asset5, newPrice, newPrice, spread, 0)
  ...
  it('should reset state when no request for more than 10 minutes', () => {
    ...
    processUpdate('ema', assetId, 100n, 100n, spread, 0)
    jest.setSystemTime(t0 + 1000)
    const afterUpdates = processUpdate('ema', assetId, 150n, 150n, spread, 0)
    ...
    const afterTimeout = processUpdate('ema', assetId, 200n, 200n, spread, 0)

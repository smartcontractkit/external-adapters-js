...
export class ImpliedPriceTransport extends SubscriptionTransport<BaseEndpointTypes> {
  ...
  async handleSingleRequest(params: RequestParams): Promise<void> {
    try {
      ...
      const divisorSources = parseSources(normalizedParams.divisorSources)

      const parsedDividendInput = parseInput(normalizedParams.dividendInput, 'dividendInput')
      const parsedDivisorInput = parseInput(normalizedParams.divisorInput, 'divisorInput')
      ...
      const dividendMinAnswers = normalizedParams.dividendMinAnswers || 1
      const divisorMinAnswers = normalizedParams.divisorMinAnswers || 1
      const operation = normalizedParams.operation.toLowerCase()

...
export async function main(): Promise<void | string> {
  try {
      ...
      adaptersToIgnore.push(
        ...allAdapters.filter((a) => {
          return (
            ...
            !options.adapters.includes(a.descopedName.replace(/-adapter$/, ''))
          )
        }),
      )
    }
    ...
    const sdlcAccountId = awsProfileSdlc['sso_account_id']

    console.log('dskloetx sdlcAccountId', sdlcAccountId)
